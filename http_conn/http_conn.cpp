...
void addfd(int epollfd, int fd, bool one_shot) {
    ...
    //判断写入，异常断开会发送一个EPOLIN EPOLLRDHUP，可以直接判断异常断开。
...
http_conn::HTTP_CODE http_conn::parse_request_line(char * text) {   //解析HTTP请求首行
    ...
    *m_url++ = '\0';
    ...
    char * method = text;
    if(strcmp(method, "GET") == 0) {    //忽略大小写比较
        m_method = GET;
    } else {
        return BAD_REQUEST;
    }
...
http_conn::LINE_STATUS http_conn::parse_line() {
    ...
    for( ; m_checked_index < m_read_index; ++m_checked_index) {
        ...
        if(temp == '\r') {
            if(m_checked_index + 1 == m_read_index) {//  \r后面就没有了，只读到这里，所以就不完整
                //printf("LINE_OPEN\n");
                return LINE_OPEN;
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    if(stat(m_real_file, &m_file_stat) < 0) {//同时把文件信息放在m_file stat上
        return NO_RESOURCE;
    }
        ...
        case FILE_REQUEST:
            {
                ...
                if (m_file_stat.st_size != 0)
                {
                    ...
                    bytes_to_send = m_write_index + m_file_stat.st_size;  //总共的大小
                    return true;
                }
                else
                {
